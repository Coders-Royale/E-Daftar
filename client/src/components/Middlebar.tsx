      ...
      <div>
        <div className="flex gap-4 items-center rounded-lg mb-6">
          <div className="bg-white-gray-150 border border-gray-450 rounded-lg h-11 flex flex-row w-full">
            ...
            <input className='bg-gray-100 h-8 my-auto mx-4 px-2 text-sm w-full' placeholder='Search Here' onChange={(e) => setDocId(e.target.value)}></input>
          </div>
        </div>
      </div>

      <div>
        {displayRooms.map((item, index) => docId.length > 0 ? (docId === item._id && <div key={index} onClick={() => setSelected(index)}><MiddleBarComponent image={Man} title={item.conversationName} name={findName(item)} time={findTime(item.updatedAt)} content={`Document: ${item.documentId}`} attachment={true} selected={true} /></div> ) :
